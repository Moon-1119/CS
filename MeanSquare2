...
namespace Minsquare
 ...
 /*               int[] values_X = new int[data_max];     // 데이터의 크기만큼
                ...
                // 문자형을 int형으로 초기화
                input_data_value(input_value, SEPERATOR, data_max, arrayX, arrayY);

                int data_num = arrayX.Length;
                int sum_X = 0;
                int sum_Y = 0;
                ...
                Calculate(arrayX, arrayY, data_num, ref sum_X, ref sum_Y, out avg_X, out avg_Y, ref sumDif_XY, ref sumDif_X2, out a, out b);
            ...
            private static void input_data_value(string[] input_value, char SEPERATOR, int data_max, ArrayList arrayX, ArrayList arrayY)
            {
                for (int i = 0; i < data_max; i++) // 읽어 드린 데이터를 배열에 세팅
                {
                    ...
                    arrayX.Add(Int16.Parse(x));
                    arrayY.Add(Int16.Parse(y));
                }
            }
            private static void Calculate(ArrayList arrayX, ArrayList arrayY, int data_num, ref int sum_X, ref int sum_Y, out float avg_X, out float avg_Y, ref float sumDif_XY, ref float sumDif_X2, out float a, out float b)
            {
                ...
                for (int i = 0; i < data_num; i++)
                {
                    ...
                    sumDif_X2 += ((float)(int)arrayX[i] - avg_X) * ((float)arrayX[i] - avg_X);
                }
